...
def findMaxSimilarity(query, groundTruthKnowledgeSupplied, k):
    
    factList = retrieveRelevantFacts(query, k)
    
    factScoreTupleList = []
    
    for fact in factList:
        
        similarityScore = cosine_similarity(fact, query)
        factScoreTupleList.append( (fact, similarityScore) )     

    highestRankedFact = (max(factScoreTupleList, key = lambda x:x[1]))[0]
    # print(factScoreTupleList,maxSimilarity, '\n')
...
for i in tqdm(range(len(questionList))):
    ...
    cos_val = findMaxSimilarity(currentQuestion, groundKnowledgeSupplied, 20)
...
for i in tqdm(range(len(questionList))):
    ...
    cos_val = findMaxSimilarity(currentQuestion, groundKnowledgeSupplied, 40)
...
for i in tqdm(range(len(questionList))):
    ...
    cos_val = findMaxSimilarity(currentQuestion, groundKnowledgeSupplied, 60)
...
for i in tqdm(range(len(questionList))):
    ...
    cos_val = findMaxSimilarity(currentQuestion, groundKnowledgeSupplied, 80)
...
for i in tqdm(range(len(questionList))):
    ...
    cos_val = findMaxSimilarity(currentQuestion, groundKnowledgeSupplied, 100)
...
for i in tqdm(range(len(questionList))):
    ...
    cos_val = findMaxSimilarity(currentQuestion, groundKnowledgeSupplied, 120)
